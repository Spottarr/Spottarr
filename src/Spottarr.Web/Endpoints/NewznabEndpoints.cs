...
internal static class NewznabEndpoints
{
    ...
    public static void MapNewznab(this IEndpointRouteBuilder app) =>
        app.MapGroup(PathPrefix).WithTags("Newznab").MapNewznab();

    private static RouteHandlerBuilder MapNewznabSearch(
        this RouteGroupBuilder group,
        string route
    ) =>
        group
            .MapGet(route, SearchHandler)
    ...
    private static void MapNewznab(this RouteGroupBuilder group)
    {
        ...
        group
            .MapGet(
                ...
                {
                    var xml = CapabilitiesHelper.GetCapabilities(
                        request.GetApiUri().Uri,
                        request.GetLogoUri().Uri,
                        env.ApplicationName,
        ...
        group
            .MapGet(
                "/get",
                async (
                    [FromQuery(Name = "guid")] int id,
                    ISpotnetAttachmentService spotnetAttachmentService,
    ...
    )
    {
        ...
        var filter = new SpotSearchFilter
        {
            ...
            ImdbId =
                imdbId == null || imdbId.StartsWith("tt", StringComparison.OrdinalIgnoreCase)
                    ? imdbId
                    : $"tt{imdbId}",
        };
    ...
    private static async Task<IResult> ExecuteSearch(
    ...
    )
    {
        ...
        var items = results
            .Spots.Select(s =>
                s.ToSyndicationItem(request.GetDetailsUri(s.Id).Uri, request.GetNzbUri(s.Id).Uri)
            )
            .ToList();
